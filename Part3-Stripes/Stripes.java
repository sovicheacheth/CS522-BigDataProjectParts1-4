			...
			for (int wordIndex = 0; wordIndex < words.length - 1; wordIndex++) {
				...
				for (int neighbor = wordIndex + 1; neighbor < words.length; neighbor++) {
					...
					if (words[neighbor].equals(words[wordIndex]))
						...
						stripe.put(
								words[neighbor],
								stripe.containsKey(words[neighbor]) ? stripe
										.get(words[neighbor]) + 1 : 1);
				}
				...
				if (!stripe.isEmpty()) {
					...
					for (Entry<String, Integer> entry : stripe.entrySet()) {
						if (entry.getKey() != null && entry.getValue() != null)
							mapWritable.put(new Text(entry.getKey()),
		...
		public void reduce(Text word, Iterable<MapWritable> values,
			...
			for (MapWritable mwNeighbor : values) {
				for (Writable neighbor : mwNeighbor.keySet()) {
					IntWritable occurences = (IntWritable) mwNeighbor
							.get(neighbor);
					count += occurences.get();

					if (mwNeighborGather.containsKey(neighbor)) {
						IntWritable tmp = (IntWritable) mwNeighborGather
								.get(neighbor);
			...
			for (Writable neighbor : mwNeighborGather.keySet()) {
				IntWritable occurrences = (IntWritable) mwNeighborGather
						.get(neighbor);
				DoubleWritable freq = new DoubleWritable(occurrences.get()
						/ count);
				Hf.put(neighbor, freq);

				sHf += "(" + neighbor.toString() + ","
						+ String.valueOf(occurrences.get()) + "/"
